...
namespace Viyrex.RuntimeServices.Callable
{
    ...
    partial class Constraint<TConstraint>
    {
        ...
        /// 鎖定回傳類型的結構
        /// </summary>
        /// <typeparam name="TReturn">回傳的結果型態</typeparam>
        public struct LockedReturn<TReturn> : IConstraintCollector<TConstraint> where TReturn : TConstraint
        {
            #region Constructors

            /// <summary>
            /// 初始化 <see cref="LockedReturn{TReturn}"/> 結構的新執行個體
            /// </summary>
            /// <param name="collector"></param>
            internal LockedReturn(Constraint<TConstraint> collector, bool thrownOrDefault)
            {
            ...
            private TReturn LazyBinding(params object[] args)
            {
                ...
                var viewBag = new Bag(this.Target, args.Convert(x => x.GetType()));

                if (this.Collector._caches.TryGetValue(viewBag, out var dele))
                {
        ...
        /// </param>
        /// <returns></returns>
        public LockedReturn<TReturnType> Strict<TReturnType>(bool thrownOrDefault = false) where TReturnType : TConstraint
        {
            if (IsSupported(typeof(TReturnType)) == TreatmentMode.NotTreated)
                throw new GenericArgumentException<TReturnType>();

            return new LockedReturn<TReturnType>(this, thrownOrDefault);
        }
